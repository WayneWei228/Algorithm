...
/*
 ...
 Z C   𝚖𝚒𝚗_𝚜𝚒𝚗𝚌𝚎_𝚕𝚊𝚜𝚝[Z]=C
 X B   𝚖𝚒𝚗_𝚜𝚒𝚗𝚌𝚎_𝚕𝚊𝚜𝚝[X]=B
 
 C B        𝚖𝚒𝚗_𝚜𝚒𝚗𝚌𝚎_𝚕𝚊𝚜𝚝[C]=B
 C B A      𝚖𝚒𝚗_𝚜𝚒𝚗𝚌𝚎_𝚕𝚊𝚜𝚝[C]=A
 C B A C    𝚖𝚒𝚗_𝚜𝚒𝚗𝚌𝚎_𝚕𝚊𝚜𝚝[C]=C  
 */
...
int main() {
    ...
    for(int i = 1; i <= N; i++) {
        int curchur = s[i - 1] - 'A'; // 当前字母
        // cout << "curchur" << curchur << endl;
        for(int c = 0; c < 26; c++) min_since_last[c] = min(curchur,min_since_last[c]);
        prefix[i] = prefix[i - 1];
        if(min_since_last[curchur] < curchur) prefix[i]++;
        min_since_last[curchur] = curchur;
        // for(int c = 0; c < 26; c++) {
        //     cout << min_since_last[c] << ' ';
        // }
        // cout << endl;
    }
    // cout << endl;
    ...
    for(int i = N; i >= 1; i--) {
        ...
        for(int c = 0; c < 26; c++) min_since_last[c] = min(curchur,min_since_last[c]);
        suffix[i] = suffix[i + 1];
        if(min_since_last[curchur] < curchur) suffix[i]++;
        min_since_last[curchur] = curchur;
    }
